...
%DIF DEL /tmp/BrPd3I_manuscript.tex   Fri May 10 15:54:27 2019
%DIF ADD manuscript/manuscript.tex    Fri May 10 12:28:10 2019
...
\DIFaddbegin \DIFadd{The heavily oscillating density functions are intended only to test the algorithm to its
limits and not to emulate a real world case.
}\DIFaddend These comparisons allowed us to obtain optimal values for the distance-size and
...
density case, but a delta ratio of 0.1 is needed for the exponential and \DIFdelbegin \DIFdel{the
}\DIFdelend \DIFaddbegin \DIFadd{most }\DIFaddend sinusoidal
density functions.
These values can be extrapolated to cover most common use cases\DIFaddbegin \DIFadd{, which are simpler than
oscillating density profiles}\DIFaddend .
...
namely when the \DIFdelbegin \DIFdel{normalized }\DIFdelend \DIFaddbegin \DIFadd{normalised }\DIFaddend distance to the computation point is greater than a
...
automatic \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{for the general case.
Although many piecewise linear approximation algorithms that automate this process
exist \mbox{%DIFAUXCMD
\citep{Ketkov1969, Vandewalle1975, Imamoto2008, Ahmadi2013}}\hspace{0pt}%DIFAUXCMD
, they either require
...
}\DIFaddend Furthermore, it is well known that the use of high-degree polynomials to approximate a
...
\textit{Step 1}: Check that the tesseroid satisfies the following inequality for \DIFdelbegin \DIFdel{each
dimension }\DIFdelend \DIFaddbegin \DIFadd{its
latitudinal and longitudinal dimensions }\DIFaddend $L_i\ (i \in \{\lambda, \phi\})$\DIFdelbegin \DIFdel{of the tesseroid}\DIFdelend :
...
\begin{equation}
    \cos\psi_t =
        \sin\phi\sin\phi_t + \cos\phi\cos\phi_t\cos(\lambda - \lambda_t)\DIFdelbegin \DIFdel{.
...
\begin{equation}
    ...
    \lambda_t = \frac{\lambda_2 + \lambda_1}{2}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
...
\caption{
    ...
    tesseroid (orange dots), and the \DIFdelbeginFL \DIFdelFL{linear density function }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{straight line }\DIFaddendFL $\rho_l(r')$ (orange line).
    ...
    (b)~Second iteration of the algorithm with a new \DIFdelbeginFL \DIFdelFL{linear density function }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{straight line }\DIFaddendFL and maximum
...
We have developed a complementary discretization algorithm in the \DIFdelbegin \DIFdel{redial }\DIFdelend \DIFaddbegin \DIFadd{radial }\DIFaddend direction that
...
\textit{Step 1}:
Define a \DIFdelbegin \DIFdel{linear function }\DIFdelend \DIFaddbegin \DIFadd{straight line }\DIFaddend $\rho_l(r')$ that assumes the same values as the normalised
density function $\rho_n(r')$ at the boundaries of the tesseroid ($r_1$ and $r_2$):

\begin{equation}
    \rho_l(r') =
    \frac{ \rho_n(r_2) - \rho_n(r_1) }{ r_2 - r_1 } (r' - r_1) + \rho_n(r_1)\DIFdelbegin \DIFdel{,
    }\DIFdelend \DIFaddbegin \DIFadd{.
    }\DIFaddend \label{eq:density-reference-line}
\end{equation}

\textit{Step 2}:
Evaluate the \DIFdelbegin \DIFdel{normalized and linear density functions }\DIFdelend \DIFaddbegin \DIFadd{normalised density function and the straight line }\DIFaddend on a range of $N$ radii
between $r_1$ and $r_2$.
We have opted for $N = 101$ but the specific value of $N$ is not critical to the
algorithm.

\textit{Step 3}:
Compute the absolute difference between the values of the \DIFdelbegin \DIFdel{linear and normalised density
functions}\DIFdelend \DIFaddbegin \DIFadd{straight line and the
normalised density function}\DIFaddend :
...
On the first iteration, the ratio \DIFdelbegin \DIFdel{$L_r/L_r^\text{orig} = 1$ }\DIFdelend \DIFaddbegin \DIFadd{$L_r/L_r^\text{orig}$ equals one }\DIFaddend because the tesseroid
...
\DIFdelbegin \DIFdel{We derive an expression }\DIFdelend \DIFaddbegin \DIFadd{Applying the Newton's Shell Theorem \mbox{%DIFAUXCMD
\citep{Chandrasekhar1995, Binney2008}}\hspace{0pt}%DIFAUXCMD
,
we derive expressions }\DIFaddend for the gravitational potential of a spherical shell with
\DIFdelbegin \DIFdel{an arbitrary density function in depth (Eq.~\ref{eq:shell-pot}),
from which we obtained
expressions for }\DIFdelend linear, exponential, and sinusoidal density functions
...
functions.
The \DIFaddbegin \DIFadd{sinusoidal density case is included in order to test the numerical approximation to
its limits.
The }\DIFaddend comparisons are repeated for all combinations of tesseroid models in
...
\DIFaddbegin \DIFadd{Furthermore, acquiring an optimal value for the $\delta$ ratio in case of unrealistic
...
of 1 and 2.5 for the potential \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend its gradient, respectively.
...
discretization.
\DIFaddbegin \DIFadd{It worth noting that the sinusoidal density tests were carried out in order to test the
algorithm to the extreme and not to emulate a real world scenario.
}\DIFaddend Values of $\delta$ equal to 0.1 are sufficient to guarantee a 0.1\% accuracy for most
...
Smaller values of $\delta$ can \DIFaddbegin \DIFadd{be }\DIFaddend used for more highly variable density functions to
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.5\linewidth]{figures/spherical-shell.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Spherical shell with inner and outer radii $R_1$ and $R_2$, respectively.
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:spherical-shell}
%DIFDELCMD < \end{figure}
%DIFDELCMD <

%DIFDELCMD < %%%
\DIFdelend Consider a spherical shell with inner radius $R_1$ and outer radius $R_2$,
whose density is a function $\rho(r')$ of the radial coordinate\DIFdelbegin \DIFdel{(Fig.
~\ref{fig:spherical-shell}).
...
\DIFadd{According to Newton's Shell Theorem (Theorem XXXI)
\mbox{%DIFAUXCMD
\citep{Chandrasekhar1995, Binney2008}}\hspace{0pt}%DIFAUXCMD
, the }\DIFaddend gravitational potential generated by the
shell on \DIFdelbegin \DIFdel{an arbitrary external point $Q$ can be written as follows}\DIFdelend \DIFaddbegin \DIFadd{any external point is the same as it would be if its mass were concentrated on
a point located at its centre}\DIFaddend :

\begin{equation}
    V_\text{sh}(\phi, \lambda, r) = \DIFdelbegin \DIFdel{G
    \int}%DIFDELCMD < \limits%%%
\DIFdel{_0^{2\pi}
    \int}%DIFDELCMD < \limits%%%
\DIFdel{_{-\frac{\pi}{2}}^\frac{\pi}{2}
    \int}%DIFDELCMD < \limits%%%
\DIFdel{_{R_1}^{R_2}
    \frac{\rho(r')}{\ell} }%DIFDELCMD < {%%%
\DIFdel{r'}%DIFDELCMD < }%%%
\DIFdel{^2 \cos\phi' \,
    dr' d\phi' d\lambda'}\DIFdelend \DIFaddbegin \DIFadd{\frac{GM}{r}}\DIFaddend ,
\end{equation}

\noindent where \DIFdelbegin \DIFdel{$\ell$ is defined in equation~\ref{eq:ell}.
}%DIFDELCMD <

%DIFDELCMD < %%%
\DIFdel{For $Q$ located along the $z$ axis (i.e., $\phi=90^\circ$) at a distance $r$ from the
origin, equation~\ref{eq:ell} simplifies to:
}%DIFDELCMD <

%DIFDELCMD < %%%
\begin{displaymath}
    \DIFdel{\ell = \sqrt{r'^2 + r^2 - 2 r r' \sin\phi'}.
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD <

%DIFDELCMD < \noindent
%DIFDELCMD < %%%
\DIFdel{Because of the rotational symmetry along the $z$ axis, the integration in $\lambda'$ is
straightforward}\DIFdelend \DIFaddbegin \DIFadd{$M$ is the total mass of the shell, which can be easily computed as
follows}\DIFaddend :

\begin{equation}
    \DIFdelbegin \DIFdel{V_\text{sh}}\DIFdelend \DIFaddbegin \DIFadd{M =
    }\iiint\limits\DIFadd{_{\Omega} \rho}\DIFaddend (r\DIFaddbegin \DIFadd{'}\DIFaddend ) \DIFaddbegin \DIFadd{dV }\DIFaddend =
    \DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend \pi \DIFdelbegin \DIFdel{G
    }\DIFdelend \int\limits\DIFdelbegin \DIFdel{_{-\frac{\pi}{2}}^\frac{\pi}{2}
    \int}%DIFDELCMD < \limits%%%
\DIFdelend _{R_1}^{R_2} \DIFdelbegin \DIFdel{\frac{\rho(r') {r'}^2 \cos\phi'}{\sqrt{r'^2 + r^2 - 2 r r' \sin\phi'}}
    \, dr}\DIFdelend \DIFaddbegin \DIFadd{\rho(r}\DIFaddend '\DIFdelbegin \DIFdel{d\phi}\DIFdelend \DIFaddbegin \DIFadd{) }{\DIFadd{r}\DIFaddend '\DIFaddbegin }\DIFadd{^2 dr'}\DIFaddend ,
\end{equation}

\noindent \DIFdelbegin \DIFdel{while the integration in $\phi'$ can be performed independently of the density function.
Making use of SymPy \mbox{%DIFAUXCMD
\citep{sympy2017}}\hspace{0pt}%DIFAUXCMD
, a Python library for symbolic mathematics, we obtained }\DIFdelend \DIFaddbegin \DIFadd{where $\Omega$ symbolizes the volume of the shell.
}

\DIFadd{Combining the two previous equations, we get }\DIFaddend the following expression for the potential:

\DIFdelbegin %DIFDELCMD < \iftwocol{
%DIFDELCMD < \begin{displaymath}
%DIFDELCMD <     \begin{split}
%DIFDELCMD <         V_\text{sh}(r) = 2\pi G
%DIFDELCMD <         \int\limits_{R_1}^{R_2}
%DIFDELCMD <         \Big[ & \sqrt{r^2 + r'^2 + 2rr'} - \\
%DIFDELCMD <         & \sqrt{r^2 + r'^2 - 2rr'}
%DIFDELCMD <         \Big] \frac{r'\rho(r')}{r} \, dr'.
%DIFDELCMD <     \end{split}
%DIFDELCMD < \label{eq:shell-pot-sqrts}
...
%DIFDELCMD < %%%
\DIFdel{Because the computation point $Q$ is outside of the shell, $r > r'$ and the square roots
in equation~\ref{eq:shell-pot-sqrts} simplify to
}%DIFDELCMD <

%DIFDELCMD < %%%
\DIFdelend \begin{equation}
    \DIFdelbegin \DIFdel{\sqrt{r^2 + r'^2 + 2rr'} }\DIFdelend \DIFaddbegin \DIFadd{V_\text{sh}(r) }\DIFaddend = \DIFdelbegin \DIFdel{|}\DIFdelend \DIFaddbegin \DIFadd{\frac{4\pi G}{r}
    \int\limits}\DIFadd{_{R_1}^{R_2} }{\DIFaddend r\DIFdelbegin \DIFdel{+ r}\DIFdelend '\DIFdelbegin \DIFdel{| = }\DIFdelend \DIFaddbegin }\DIFadd{^2 \rho(}\DIFaddend r\DIFdelbegin \DIFdel{+ r}\DIFdelend '\DIFaddbegin \DIFadd{) \, dr'}\DIFaddend ,
\DIFaddbegin \label{eq:shell-pot}
\DIFaddend \end{equation}

\DIFdelbegin \begin{displaymath}
    \DIFdel{\sqrt{r^2 + r'^2 - 2rr'} = |r - r'| = r - r',
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD <

%DIFDELCMD < %%%
\DIFdelend \noindent which \DIFdelbegin \DIFdel{leads to the following expression for the potential:
}\DIFdelend \DIFaddbegin \DIFadd{is in agreement with the one obtained by \mbox{%DIFAUXCMD
\citet[p.62]{Binney2008}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend

\DIFdelbegin \begin{displaymath}
    \DIFdel{V_\text{sh}(r) = \frac{4\pi G}{r}
    \int\limits_{R_1}^{R_2} }{\DIFdel{r'}}\DIFdel{^2 \rho(r') \, dr'.
%DIFDELCMD < \label{eq:shell-pot}%%%
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD <

%DIFDELCMD < %%%
\DIFdelend The gradient of potentials that depends solely on $r$ have only one non zero components:
the vertical component of the gradient ($g_z$).
Following \citet{Grombein2013}:

\begin{equation}
    g_z(r) = \frac{V_\text{sh}(r)}{r}\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend \label{eq:shell-gz}
\end{equation}

From Eq.~\ref{eq:shell-pot} we can obtain expressions of the gravitational potential for
different density functions.
\DIFaddbegin \DIFadd{The integration of the following density functions have been carried out by SymPy
\mbox{%DIFAUXCMD
\citep{sympy2017}}\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD <     V_\text{sine}(r) = \frac{4\pi G}{r} \frac{A}{k^3} \Big[
%DIFDELCMD <     & (2 - k^2 R_2^2) \cos(k(R_2 - R)) + 2 k R_2 \sin(k(R_2 - R)) - \\
%DIFDELCMD <     & (2 - k^2 R_1^2) \cos(k(R_1 - R)) - 2 k R_1 \sin(k(R_1 - R))
%DIFDELCMD <         \Big]
%DIFDELCMD <     \end{split}%%%
