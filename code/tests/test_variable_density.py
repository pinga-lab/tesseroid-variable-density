...
def density_fun(height):
    return 2600


def density_fun_1(height):
    return -1000


def density_fun_2(height):
    return 5000
...
def test_warn_if_division_makes_too_small():
    ...
    models = [
        ...
        [Tesseroid(-1e-3, 1e-3, -1e-3, 1e-3, 0, -1e-2,
                   {'density': density_fun})]]
...
def test_serial_vs_parallel():
    ...
    model.addprop('density', [density_fun for i in range(500)])
...
def test_fails_if_shape_mismatch():
    ...
    model = [Tesseroid(0, 1, 0, 1, 1000, -20000, {'density': density_fun})]
    area = [-2, 2, -2, 2]
    shape = (51, 51)
...
def test_tesseroid_vs_spherical_shell():
    ...
    density = density_fun(0)
    top = 1000
    bottom = 0
    model = TesseroidMesh((0, 360, -90, 90, top, bottom), (1, 6, 12))
    model.addprop('density', [density_fun for i in range(model.size)])
    h = 10000
...
def test_skip_none_and_missing_properties():
    ...
    model = [Tesseroid(0, 1, 0, 1, 1000, -20000, {'density': density_fun}),
             ...
             None,
             Tesseroid(-1.5, -0.5, -1.5, -1, 1000, -20000,
                       {'density': density_fun, 'magnetization': [1, 2, 3]}),
             None]
    puremodel = [Tesseroid(0, 1, 0, 1, 1000, -20000, {'density': density_fun}),
                 Tesseroid(-1.5, -0.5, -1.5, -1, 1000, -20000,
                           {'density': density_fun})]
    area = [-2, 2, -2, 2]
    shape = (51, 51)
...
def test_overwrite_density():
    ...
    model = [Tesseroid(0, 1, 0, 1, 1000, -20000, {'density': -1000})]
    density = density_fun(0)
    other = [Tesseroid(0, 1, 0, 1, 1000, -20000, {'density': density_fun})]
    area = [-2, 2, -2, 2]
    shape = (51, 51)
...
def test_stack_overflow():
    ...
    model = [Tesseroid(0, 1, 0, 1, 0, -20e4, {'density': density_fun})]
    area = [0, 1, 0, 1]
    shape = [20, 20]
